...
from models.entities import CourseCreate, CourseResponse
...
class CourseAPIClient:
    ...
    def update_course(self, course_id: int, course: CourseCreate) -> CourseResponse:
        response = requests.put(f"{BASE_URL}{course_id}/", json=course.model_dump())
        response.raise_for_status()
        return CourseResponse(**response.json())

    def partial_update_course(self, course_id: int, data: dict) -> CourseResponse:
        response = requests.patch(f"{BASE_URL}{course_id}/", json=data)
        response.raise_for_status()
        return CourseResponse(**response.json())
